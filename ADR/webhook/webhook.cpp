...
Webhook::Webhook(std::string token, long long id) {
  this->url =
      "https://discord.com//api/webhooks/" + std::to_string(id) + "/" + token;
}
...
void Webhook::AddContent(const std::string& raw) { this->content << raw; }
...
bool Webhook::Send() {
  ...
  curl_formadd(&formpost, &lastptr, CURLFORM_COPYNAME, "payload_json",
               CURLFORM_COPYCONTENTS, jsonPayload.c_str(), CURLFORM_END);

  for (const std::string& file : this->files) {
    curl_formadd(&formpost, &lastptr, CURLFORM_COPYNAME,
                 HPathGetName(file).c_str(), CURLFORM_FILE, file.c_str(),
                 CURLFORM_END);
  }
...
void WebhookEmbed::SetURL(const std::string& url) { this->embed["url"] = url; }
...
void WebhookEmbed::SetTimestamp() {
  ...
  std::stringstream st;
  st << buffer << "." << std::setw(3) << std::setfill('0')
     << milliseconds.count() << "Z";
  this->embed["timestamp"] = st.str();
}

void WebhookEmbed::SetColor(int color) { this->embed["color"] = color; }

void WebhookEmbed::SetAuthor(const std::string& name, const std::string& url,
