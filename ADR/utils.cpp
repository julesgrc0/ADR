...
std::string HPathGetWinFld(REFKNOWNFOLDERID id)
{
    ...
    if (SUCCEEDED(result))
    {
        int size_needed = WideCharToMultiByte(CP_UTF8, 0, pwPath, -1, NULL, 0, NULL, NULL);
        path.resize(size_needed - 1);
        WideCharToMultiByte(CP_UTF8, 0, pwPath, -1, &path[0], size_needed, NULL, NULL);
    }
...
std::string HPathAdd(const std::string& base, const std::string& path, bool isdir)
{
...
{
    return GetFileAttributesA(path.c_str()) != INVALID_FILE_ATTRIBUTES && GetFileAttributesA(path.c_str()) & FILE_ATTRIBUTE_DIRECTORY;
}
...
std::vector<std::string> HChunkFile(const std::string& path, size_t chunkSize)
{
    ...
    {
        std::string chunkpath = base + HPathGetName(path) + "_" + std::to_string(i + 1) + ".bin";
